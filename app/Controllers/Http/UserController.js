  ...
  async store({auth, request, response}) {
    ...
    try {
      ...
      if (Env.get('ACCOUNT_CONFIRM', 'true') === 'true') {
        ...
        const verify = {domain: Env.get('MAIL_URL', '127.0.0.1:3333'), token: encodeURI(token)}
  ...
  async login({auth, request, response}) {
    ...
    try {
      ...
      if (user.active) {
        ...
        Logger.info('Login - User deactivated %s', username)
        return response.send('User deactivated - Please confirm your E-Mail!')
      }
    } catch (err) {
      Logger.error('Login - Error: %s', err)
      Logger.info('Login - Can not find User credentials for %s', username)
